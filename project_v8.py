...
def getMuseEmotion(app):
    ...
    if not app.calibrationDone:
        ...
        if len(app.arousalCalibrationSamples) >= app.calibrationSteps:
            app.arousalBaseline = sum(app.arousalCalibrationSamples) / len(app.arousalCalibrationSamples)
    ...
    arousal = max(0, min(1, (arousalRaw - app.arousalBaseline) / app.arousalBaseline + 0.5))
    ...
    VALENCE_OFFSET = 0.20
    valence = max(-1, min(1, valenceAdjusted * 0.7 + VALENCE_OFFSET))    
    #valence = max(-1, min(1, valenceRaw * 0.75 + VALENCE_OFFSET))
...
#from Youtube Video
def am_synthesis(
        carrier_frequency: float, #og wave's freq
        modulator_wave: np.array, #modulator
        modulation_index: float = 0.5, #how much modulator affects carrier
        amplitude: float = 0.5, #peak amp
        sample_rate: int=44100
        ) -> np.ndarray:
    #determine # total samples
    total_samples = len(modulator_wave)

    #.() makes sequence of indices from 0 until #samples. 
    # divide by sample rate to get time vals

    time_points = np.arange(total_samples) / sample_rate

    #generate carrier wave - calculate position at each moment in time
    carrier_wave = np.sin(2 * np.pi * carrier_frequency * time_points)

    # modulate oscillates -1 to 1, BUT -1 causes carrier to invert phase.
    # so, add 1 
    ...
    am_wave = amplitude * (am_wave / max_amplitude) #division normalizes it so its peak is 1
