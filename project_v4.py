...
def calmScale(app):
    ...
    if app.steps % app.stepsPerSecond == 0:
        print(pentatonicScale[random1], pentatonicScale[random2], app.duration)
...
def sadScale(app):
    ...
    app.mySound2 = sine_tone(app, frequency = freq2, amplitude = 0.1, duration = newDuration)
    attack = 0.5
    decay = 0.3
    sustain = 0.6
    release = 0.5
    ...
    random3 = random1 + 5 #sixth
    if app.steps % app.stepsPerSecond == 0:
        print(aHarmonicMinor[random1], aHarmonicMinor[random2], app.duration)
